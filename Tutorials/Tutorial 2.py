...
# Function calls
im = cv2.imread("1.jpg")  # Read the image in directory "samples/00003_09.jpg" into a numpy nd array
# result = im.copy()
# px = im.copy()
ax = plt.axes(projection='3d')  # Set the visualization of matplotlib to 3d shape(cube)
...
expected_colors = [(168, 168, 168)]
replacing_color = (0, 0, 0)  # Sometimes the R, G, B channels are switched to B, G, R on some platforms,
...
width, height, _ = im.shape  # Get the width and height of the image, without the channels = _
for row in range(0, height):  # For row in range 0 to maximum,
    for col in range(0, width):  # for col in range 0 to maximum,
        ...
        NewCol = (pix[0] / 255, pix[1] / 255, pix[2] / 255)  # Scale the 3 channels to a 0-1 interval

        if (not NewCol in c):  # if the color is not already in the c list,
            x.append(pix[0])  # append the first channel,
...
ax.scatter(x, y, z, c=c)  # we visualize the colors in the old image on a 3d scale(cube),
plt.show()  # we show the 3d cube
